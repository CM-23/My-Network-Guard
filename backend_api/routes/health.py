...
def diagnostics():
    ...
    # Telegram status
    telegram_ok = (
        bool(cfg.telegram.bot_token)
        and len(database.execute_read("SELECT chat_id FROM telegram_subscribers")) > 0
    )
    ...
    health = {
        ...
        "discovery": {
            "status": _status(
                diag["passive_discovery_active"] or diag["active_arp_active"]
            ),
...
def status():
    ...
    from sniffer import is_elevated, get_scan_mode
    try:
        import psutil
    ...
    q_size    = 0  # Injected externally; placeholder
...
def prometheus_metrics():
    ...
    alert_count  = database.execute_read(
        "SELECT count(*) as c FROM alerts WHERE is_resolved=0"
    )[0]["c"]
    traffic_rows = database.execute_read(
        "SELECT count(*) as c FROM traffic_logs"
    )[0]["c"]
    diag = sniffer.get_diagnostics()

    lines = [
        f"# HELP nids_devices_total Total discovered devices",
        f"# TYPE nids_devices_total gauge",
        f"nids_devices_total {device_count}",
        f"# HELP nids_alerts_active Active (unresolved) alerts",
        f"# TYPE nids_alerts_active gauge",
        f"nids_alerts_active {alert_count}",
        f"# HELP nids_packets_captured Total packets captured",
        f"# TYPE nids_packets_captured counter",
        f"nids_packets_captured {diag['packets_captured']}",
        f"# HELP nids_traffic_flows Total traffic flow records",
        f"# TYPE nids_traffic_flows gauge",
