...
from shared.validators import validate_alert_id, validate_severity
...
def list_alerts():
    ...
    total = database.execute_read(
        f"SELECT count(*) as c FROM alerts {where}",
        tuple(params)
    )[0]["c"]
...
def resolve_alert(alert_id: int):
    ...
    database.record_audit(
        action="RESOLVE_ALERT", entity="alerts", entity_id=str(alert_id),
...
def resolve_all_alerts():
    result = database.execute_write_sync(
        "UPDATE alerts SET is_resolved = 1 WHERE is_resolved = 0"
    )
    count = result.get("rowcount", 0)
    database.record_audit(
        action="RESOLVE_ALL_ALERTS", entity="alerts",
        new_value=str(count), actor="user",
...
def alert_stats():
    ...
    type_dist = database.execute_read(
    ...
    daily_trend = database.execute_read(
    ...
    top_affected = database.execute_read(
        """SELECT affected_mac, count(*) as count
           FROM alerts
