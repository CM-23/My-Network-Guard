...
def network_stats():
    ...
    online_count  = database.execute_read(
        "SELECT count(*) as c FROM devices WHERE is_online=1 AND deleted_at IS NULL"
    )[0]["c"]
    alert_total   = database.execute_read(
        "SELECT count(*) as c FROM alerts"
    )[0]["c"]
    alert_active  = database.execute_read(
        "SELECT count(*) as c FROM alerts WHERE is_resolved=0"
    )[0]["c"]
    traffic_flows = database.execute_read(
    ...
    device_types  = database.execute_read(
        """SELECT device_type, count(*) as count
           FROM devices WHERE deleted_at IS NULL
           GROUP BY device_type
           ORDER BY count DESC"""
    )
    recent_alerts = database.execute_read(
        """SELECT id, timestamp, alert_type, severity, description, is_resolved
           FROM alerts
...
def traffic_logs():
    ...
    total = database.execute_read(
        f"SELECT count(*) as c FROM traffic_logs {where}",
        tuple(params)
    )[0]["c"]
...
def top_talkers():
    ...
    rows = database.execute_read(
        """SELECT source_ip, SUM(packet_count) as total_packets
           FROM traffic_logs
           GROUP BY source_ip
...
def protocol_distribution():
    ...
    rows = database.execute_read(
        """SELECT protocol, SUM(packet_count) as total_packets, count(*) as flows
           FROM traffic_logs
           GROUP BY protocol
           ORDER BY total_packets DESC"""
    )
