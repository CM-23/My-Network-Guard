...
def auto_detect_interface():
    ...
    try:
        ...
        forbidden = ["loopback", "docker", "virtualbox", "vmware", "wsl", "hyper-v", "bluetooth", "teredo", "host-only", "tap-"]
...
def start_sniffer(packet_queue, interface=None, simulation_mode=False):
    ...
    if active_ok:
        ...
        logger.warning(f"Active scanning disabled: {active_reason}. Falling back to passive sniffing + ARP cache reader.")
...
def trigger_arp_scan(packet_queue):
    ...
    if _simulation_mode:
        ...
        return
    if check_npcap_missing():
        raise RuntimeError("Npcap is not installed. Real network discovery is disabled. Install Npcap with 'WinPcap API-compatible Mode' enabled and restart the application.")
    if not scapy_available:
    ...
    try:
        ...
        if responded_macs:
            ...
            database.execute_write_async(
                f"UPDATE devices SET is_online = 0 WHERE mac_address NOT IN ({placeholders})",
        ...
        err_msg = str(e).lower()
        if "permission" in err_msg or "operation not permitted" in err_msg or "socket" in err_msg or "winpcap" in err_msg:
            raise PermissionError("insufficient permissions. Run as Administrator/sudo.")
        raise
